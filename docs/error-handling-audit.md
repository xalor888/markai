...
- `clipboard.ts:30` 的注释 `// 忽略` 与实际行为（随后弹失败提示）不一致——**行为是对的，说法是错的**，属文档层面的误导；
- `themeStore.load()` 读取失败时静默回落到 `system`：用户保存的主题被丢掉且无提示。影响是外观层面（不是数据），且与 `setTheme` 的失败提示不同口径，留作后续统一。
...
- **`clipboard.ts` 注释与行为不一致**、**`themeStore.load()` 静默回落**：见 §三「仍需改进」，未修。
